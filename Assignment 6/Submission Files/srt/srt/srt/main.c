...
static void *thread_function(void* t_num){
    ...
    /* for every pixel */
    for( int px=thread_num; px<width; px += nthreads )
    {
...
int main( int argc, char **argv )
{
    ...
    int* thread_nums = malloc(nthreads*sizeof(int));

    for(int i = 0; i < nthreads; i++){
      thread_nums[i] = i;
      int create_result = pthread_create(&threads[i], NULL, thread_function, (void*)(&thread_nums[i]));
      if(create_result != 0){
	fprintf(stderr, "There was an error creating the thread!");
      }
    }
